...
def main():
    ...
    if False:
        ...
        era.to_netcdf(tmp_path+'era_fig2_tmp_abs.nc')
        nk.to_netcdf(tmp_path+'nk_fig2_tmp_abs.nc')
        bw.to_netcdf(tmp_path+'bw_fig2_tmp_abs.nc')

    else:

        era = xr.open_dataarray(tmp_path+'era_fig2_tmp_abs.nc')
        nk  = xr.open_dataarray(tmp_path+'nk_fig2_tmp_abs.nc')
        bw  = xr.open_dataarray(tmp_path+'bw_fig2_tmp_abs.nc')
    ...
    latex.save_figure(fig,'/nird/home/heau/figures_paper/Figure2_absolute')
    ...
    if False:
        ...
        for ax,loc,name in zip(axes.T,bw.location,bw.loc_name.values):
            ...
            era_ = era_.where(np.isfinite(era_), drop=True)
            nk_ = nk_.where(np.isfinite(nk_), drop=True)
            bw_ = bw_.where(np.isfinite(bw_), drop=True)

            era_ = era_.where(era_.time.isin(bw_.time), drop=True)
            nk_  = nk_.where(nk_.time.isin(bw_.time), drop=True)

            era_ = era_.where(era_.time.isin(nk_.time), drop=True)
            nk_  = nk_.where(nk_.time.isin(era_.time), drop=True)

            bw_  = bw_.where(bw_.time.isin(era_.time), drop=True)
            ...
            ax[0].hist(bw_,label='BarentsWatch',density=False,color='C2',bins=bins)
            #ax[0].set_xlim([-4,4])
            if name=='Ljonesbjørgene':
                ax[0].legend(fontsize=5,loc='upper right')
                #ax[0].set_ylabel('Sample fraction')
            ax[1].hist(nk_,label='Norkyst',density=False,color='C0',bins=bins)
            #ax[1].set_xlim([-4,4])
            if name=='Ljonesbjørgene':
                ax[1].legend(fontsize=5,loc='upper right')
                ax[1].set_ylabel('Count')
            ax[2].hist(era_,label='ERA5',density=False,color='C1',bins=bins)
            #ax[2].set_xlim([-4,4])
            if name=='Ljonesbjørgene':
                ax[2].legend(fontsize=5,loc='upper right')
                #ax[2].set_ylabel('Sample fraction')

            ax[2].set_xlabel('[$^\circ$C]')



            ax[0].set_title(name,fontsize=10)

        latex.save_figure(fig,'/nird/home/heau/figures_paper/Figure3_3rows')
        ...
        for ax,loc,name in zip(axes.T,bw.location,bw.loc_name.values):
            ...
            ax.hist(bw_,label='BarentsWatch',density=False,color='C2',bins=bins,histtype=u'step',)
            ax.hist(nk_,label='Norkyst',density=False,color='C0',bins=bins,histtype=u'step')
            ax.hist(era_,label='ERA5',density=False,color='C1',bins=bins,histtype=u'step')
        ...
        latex.save_figure(fig,'/nird/home/heau/figures_paper/Figure3')

...
def calc():
    ...
    if False:
        ...
        if False:
            ...
            combo_a.to_netcdf(tmp_path+'tmp_comboa_fig7_hk.nc')
        ...
        crps_ref.to_netcdf(tmp_path+'tmp_crps_clim_fig7_hk.nc')
        ...
        pers.to_netcdf(tmp_path+'tmp_pers_fig7_hk.nc')

    ec = hindcast.data_a - hindcast.data_a.mean('member')
    ec = ec /hindcast.data_a.std('member')
    ec = ec + hindcast.data_a.mean('member')

    #combo   = xr.open_dataarray(tmp_path+'tmp_comboa_fig7_hk.nc')

    # use without spread adjustment
    combo   = xr.open_dataarray(tmp_path+'tmp_comboa_fig7_hk.nc')
    combo_u = hindcast.data_a - hindcast.data_a.mean('member')
    combo_u = combo_u / combo_u.std('member')
    combo = combo_u + combo

    pers      = xr.open_dataarray(tmp_path+'tmp_pers_fig7_hk.nc')
    mse_pers  = xs.mae(observations.data_a,pers,dim=[]).rename('mse')
...
def main():
    ...
    crps_co_,crps_fc_,mse_pers_  = calc()
    crps_ref_ = xr.open_dataset(tmp_path+'tmp_crps_clim_fig7_hk.nc').crps

    for month in np.arange(4,13,1):

        crps_co = crps_co_.where(crps_co_.time.dt.month==month,drop=True)
        crps_fc = crps_fc_.where(crps_fc_.time.dt.month==month,drop=True)
        crps_ref = crps_ref_.where(crps_ref_.time.dt.month==month,drop=True)
        mse_pers = mse_pers_.where(mse_pers_.time.dt.month==month,drop=True)

        mse_pers = mse_pers.mean('time')
        crps_co  = crps_co.mean('time')
        crps_fc  = crps_fc.mean('time')
        crps_ref = crps_ref.mean('time')
        ...
        for _ax_,crps,label,c in zip(axes,[crps_ref,crps_fc,mse_pers,crps_co],['CLIM','EC','PERS','COMBO'],['darkblue','darkgreen','darkred','orange']):
            ...
            for ax,step,col in zip(_ax_,steps,range(len(steps))):
                ...
                feature = ax.add_feature(coast, edgecolor='k', linewidth=0.2, zorder=40)
        cbar=fig.colorbar(
            cs,
        ...
        fig.suptitle('')
        plt.savefig('/nird/home/heau/figures_paper/Figure7_whole_coast_'+str(month)+'.png',bbox_inches='tight')
        #latex.save_figure(fig,'/nird/home/heau/figures_paper/Figure7_whole_coast_'+str(month))

...
def calc():
    ...
    mae_pers  = xs.rmse(observations.data_a,pers,dim=[]).rename('mae').mean('time')
    mae_combo = xs.rmse(observations.data_a,combo,dim=[]).rename('mae').mean('time')

    return mae_pers,mae_combo
...
def main():
    ...
    pers,combo = xr.align(pers,combo)
    ...
    levels = np.arange(0.5,0.825,0.025)
    norm   = BoundaryNorm(levels,cmap.N,extend='both')

    for _ax_,crps,label,c in zip(axes,[pers,combo],['PERS','COMBO'],['darkgreen','orange']):
    ...
    fig.suptitle('')
    latex.save_figure(fig,'/nird/home/heau/figures_paper/new062022_fig4_persistence_rmse')
